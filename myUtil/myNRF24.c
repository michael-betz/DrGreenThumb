...
void nRfInitProm(  uint8_t addrLength ){	//Init for Promiscuous reception 
    ...
    switch (addrLength){
        ...
        break;
        default:
        debug_str("error: addrLength must be 3, 4 or 5!\n");
        return;
    }
    initSPI();                              
	NRF_CE_OFF();
...
void nRfSendBytes( uint8_t *bytesToSend, uint8_t len, uint8_t *adr, uint8_t noAck ){
	...
	while( nTries < 30 ) {
	    ...
	    if( tempStatus & (1<<TX_DS) ){
	    	...
	    	debug_dec( nTries );
	    	debug_putc(',');
	    	debug_dec( nRfGetRetransmits() );
	    	debug_str("] ");
    	    nRfWrite_register( STATUS, (1<<TX_DS) );    //Clear Data ready flag
	        break;					//Done transmitting, break the loop
	    }
...
void nRfHexdump( void ){
	uint8_t x;
	debug_str( "All nRF24 registers:\n0x00:  " );
	for( x=0; x<=0x1D; x++){
		debug_hex( nRfRead_register(x), 2 );
		debug_putc(' ');
		if( ((x+1)%8) == 0 ){
			debug_str("\n0x");
			debug_hex( x+1, 2 );
